利用C++的class做的一个List，没有用模板，只实现了存储int类型的数据，首先开辟的头结点粗放的数据为**链表的长度**，头结点除外，其余的结点的数据域都是存放数据，函数如下

| 名称                      | 功能             |
| ------------------------- | ---------------- |
| pushFront                 | 头插             |
| pushBack                  | 尾插             |
| insert(int pos ,int elem) | 按下标插入       |
| popFront                  | 头删             |
| popBack                   | 尾删             |
| erase                     | 按下标删除       |
| find(int elem)            | 按值查找         |
| operator=                 | 重载赋值号       |
| swapList                  | 交换             |
| resize(int num,int elem)  | 指定大小         |
| remove(int elem)          | 按元素移除结点   |
| reverse                   | 翻转             |
| sort                      | 排序（由小到大） |
| printList                 | 打印元素及其个数 |
| front                     | 头元素           |
